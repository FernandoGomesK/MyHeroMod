...
//             {
//                 if (player.ownedProjectileCounts[ModContent.ProjectileType<ChargePaleProj>()] > 0) return;
                
//                 int phosDamage = transPlayer.CurrentStage == QuirkStage.Final ? 550 : 180;
                
//                 Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, direction, ModContent.ProjectileType<ChargePaleProj>(), (int)(phosDamage * multiplier), 0f, player.whoAmI);
//             }
//             else if (hchhPlayer.IsFlashFireFistActive)
//             {
//                 if (player.ownedProjectileCounts[ModContent.ProjectileType<ChargeJetKindlingProj>()] > 0) return;
...
//                 Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, direction, ModContent.ProjectileType<ChargeJetKindlingProj>(), (int)(fireDamage * multiplier), 0f, player.whoAmI);
...
//             else
//             {
//                 if (player.ownedProjectileCounts[ModContent.ProjectileType<ChargeJetIceProj>()] > 0) return;
...
//                 Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, direction, ModContent.ProjectileType<ChargeJetIceProj>(), (int)(iceDamage * multiplier), 0f, player.whoAmI);
